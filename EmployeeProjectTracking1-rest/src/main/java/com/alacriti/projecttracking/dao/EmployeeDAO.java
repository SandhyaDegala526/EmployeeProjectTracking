...
public class EmployeeDAO extends BaseDAO {
	...
	public boolean checkEmpStatus(Employee employee)throws DAOException
	{
		PreparedStatement stmt = null;
		ResultSet rs=null;
		...
		System.out.println("DAO 1");
			String sqlCmd="select employee_state from sandhyad_ept_employee_details where employee_id=?";
			stmt=getPreparedStatement(getConnection(), sqlCmd);
			stmt.setString(1, employee.getEmployeeId());
			...
			while(rs.next())
			{
			status=rs.getString("employee_state").equals("assigned");
			}
		}
		catch (SQLException e) {
